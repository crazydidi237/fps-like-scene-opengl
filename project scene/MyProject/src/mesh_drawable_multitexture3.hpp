...
struct mesh_drawable_multitexture3 : vcl::mesh_drawable {

	using mesh_drawable::mesh_drawable; // reuse the same constructor

	GLuint texture_2; // add a second texture
	GLuint texture_3; // add a second texture
	GLuint texture_4; // add a second texture
	GLuint texture_5; // add a second texture
};


// Adapt the draw function for a mesh_drawable_multitexture
template <typename SCENE>
void draw(mesh_drawable_multitexture3 const& drawable, SCENE const& scene)
{
	...
	vcl::opengl_uniform(drawable.shader, "image_texture_3", 2);  opengl_check;
	...
	vcl::opengl_uniform(drawable.shader, "image_texture_4", 3);  opengl_check;

	glActiveTexture(GL_TEXTURE4); opengl_check; // the additional texture 
	glBindTexture(GL_TEXTURE_2D, drawable.texture_5); opengl_check;
	vcl::opengl_uniform(drawable.shader, "image_texture_5", 4);  opengl_check;
